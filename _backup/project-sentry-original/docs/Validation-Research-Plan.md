...
- The app currently validates crash logic with synthetic data only.
- `app/lib/services/simulated_sensor_service.dart` emits random and injected spike values.
- `app/lib/services/scenario_library.dart` contains hand-authored scenarios such as `collision`, `rear_end`, `pothole_hit`, and `phone_drop`.
- `sensors_plus` is already present in `app/pubspec.yaml`, but no live sensor stream is wired into `app/lib/` yet.
- Important implementation detail: the current detection thresholds behave like `g` units, while `sensors_plus` accelerometer streams are documented in `m/s^2`. We need a clear conversion step before mixing simulated and live data.
...
From `docs/Capstone GitHub Links.md`, the most useful items for this workstream are:
